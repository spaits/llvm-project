...
bool isBackwardPropagatableCopy(const DestSourcePair &CopyOperands,
...
bool BackwardPropagationCL::isBackwardPropagatableRegClassCopy(
...
bool BackwardPropagationCL::hasImplicitOverlap(const MachineInstr &MI,
...
bool BackwardPropagationCL::hasOverlappingMultipleDef(
...
private:
  ...
  void BackwardPropagationAnalysis(MachineBasicBlock &MBB);
...
bool MachineCopyPropagation::runOnMachineFunction(MachineFunction &MF) {
  ...
  BackwardPropagationCL BP(TRI,TII,MRI,UseCopyInstr, Changed);
  for (MachineBasicBlock &MBB : MF) {
    if (isSpillageCopyElimEnabled)
      EliminateSpillageCopies(MBB);
    //BackwardCopyPropagateBlock(MBB);
    BP.backwardCopyPropagateBlock(MBB);
